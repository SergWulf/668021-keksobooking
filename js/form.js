...
(function () {
  ...
  for (var i = 0; i < window.data.formAd.children.length; i++) {
    window.data.formAd.children[i].setAttribute('disabled', 'disabled');
  }
  ...
  // Дополнительная очистка полей с данными координат метки
  ...
  var buttonResetClickHandler = function (evtReset) {
    ...
    window.data.formAd.reset();
    window.data.mapAdverts.classList.add('map--faded');
    window.data.formFilters.classList.add('ad-form--disabled');
    var blockPins = document.querySelector('.map__pins');
    for (var j = 0; j < window.data.realEstates.length; j++) {
      blockPins.removeChild(blockPins.lastChild);
    }
    window.data.mapPin.style.left = String(window.data.BEGIN_PIN_MAIN_COORDINATE_X + 'px');
    window.data.mapPin.style.top = String(window.data.BEGIN_PIN_MAIN_COORDINATE_Y + 'px');
    // Изначальные координаты метки
    window.data.formAd.querySelector('#address').setAttribute('value', window.map.showCoordinatesMapPin(window.data.mapPin, false));
    window.blockingFormFields(true);
    // Обработка события 'mouseup' через 'mousedown' на главной метке: создание меток на карте и разблокировки полей формы
    window.data.mapPin.addEventListener('mousedown', window.map.buttonMouseDownHandlerCreatePins);
  };
